...
class StaticInventory(object):
    ...
    def __init__(self, path=None, os_version=None, logger=None):
        self._log = logger or log
        self._log.debug("Instantiating static inventory")
        ...
        if path:
            self._log.debug("Loading static inventory from file %s", path)
            self._load_static_inventory(load_yaml(path))
        self._log.debug("Static inventory initialized")

    def _initialize_empty_directory(self, os_name, os_version):
        if os_version:
            self._log.debug("Initialized inventory for %s %s",
                            os_name, os_version)
            inv = {os_name: {os_version: {'hosts': {}, 'domains': {}}}}
        else:
            self._log.debug("Initialized inventory for %s", os_name)
            inv = {os_name: {'hosts': {}, 'domains': {}}}
        self._inventory.update(inv)

    def _load_static_inventory(self, inventory):
        self._log.debug("Loading content of static inventory")
        self._inventory.update(inventory)

    def _get_resource_type_dict(self, dict_name):
        try:
            self._log.debug("Accessing inventory for %s dictionary", dict_name)
            if self._os_version:
                self._log.debug("Accessing %s for os %s %s",
                                dict_name, self._os_id, self._os_version)
                return (
                    self._inventory[self._os_id][self._os_version][dict_name]
                )
            else:
                self._log.debug("Accessing %s for os %s",
                                dict_name, self._os_id)
                return self._inventory[self._os_id][dict_name]
        except KeyError as e:
            self._log.error("Malformed static inventory: %s\nFile: %s",
    ...
    def lookup_domain(self, domain_type):
        try:
            self._log.debug("Looking up domain with type %s", domain_type)
            return self.domains[domain_type]
        except KeyError:
            self._log.error("Could not find domain with type %s", domain_type)
            raise

    def lookup_host(self, role_name):
        try:
            self._log.debug("Looking up host with role %s", role_name)
            return self.hosts[role_name]
        except KeyError:
            self._log.error("Could not find host with role %s", role_name)
            raise
